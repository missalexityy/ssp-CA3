...
// This function either turns on or off the row highlighting for deli
// items (depending on the value of bShowdeli)
function highlightdeli(idTable, bShowdeli) {
    // if bShowdeli is true, then we're highlighting deli
    ...
    // "deli" attribute on it.
    for (i = 0; i < aTRs.length; i++) {
        if (aTRs[i].getAttribute('deli') && aTRs[i].getAttribute('deli') == "true") {
            if (bShowdeli) {
...
window.addEventListener("load", function () {
    ...
    document.querySelector("#showdeli").addEventListener("click", function () {
        highlightdeli('menuTable', this.checked);
    });
});